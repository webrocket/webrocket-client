...
WebRocket.prototype = {
  ...
  subscribe: function(channel, data) {
    return this.connection.subscribe(channel, data);
  },
  ...
  broadcast: function(channelName, event, trigger, data) {
    var channel = this.connection.channels.find(channelName);
    channel.broadcast(event, trigger, data);
  },
...
WebRocket.Channel.prototype = {
  ...
  broadcast: function(event, trigger, data) {
    var broadcastJson = {
      broadcast: { channel: this.name, event: event, trigger: trigger, data: data }
    };
    ...
    switch(true) {
      ...
      default:
        ...
        if(!!channel) {
          ...
          /*
          channel.events();
          var message = new WebRocket.Message({
            sid: data[event].sid,
            channel: channelName
          });
          */
